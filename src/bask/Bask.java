...
public class Bask {
    ...
    public static void main(String[] args) {
        try {
            ...
            int[][] pixelVotes=new int[binIm.getWidth()][binIm.getHeight()];
            int maxPixelVote=0;
    ...
    static void cluster(int[][] pixelVotes, int x, int y) {
    	...
    	while (!notDone.isEmpty()) {
    		...
    		centreX+=p.x*pixelVotes[p.x][p.y];
    		centreY+=p.y*pixelVotes[p.x][p.y];
    		total+=pixelVotes[p.x][p.y];
    		for (int xx=-1; xx<=1; xx++) {
    			for (int yy=-1; yy<=1; yy++) {
        			...
        			if (pixelVotes[p2.x][p2.y]==0) continue;
        			notDone.push(p2);	
    			}
    		}
    	...
    	for (Point p: done) {
    		pixelVotes[p.x][p.y]=0;
    	}
    	pixelVotes[centreX][centreY]=total;
    }
}
